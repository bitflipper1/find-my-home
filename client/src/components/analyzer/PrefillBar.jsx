    ...
    <section className="bg-white rounded-xl border border-gray-200 shadow-sm p-3">
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={onApplyBenchmark} disabled={!hasBenchmark}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-700 border border-[#4395FC] rounded-lg hover:bg-blue-50 disabled:opacity-40"
        ...
        <button onClick={onApplyDefaults} disabled={!hasMarket}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-600 border border-[#8C95A7] rounded-lg hover:bg-gray-50 disabled:opacity-40"
        ...
        <button onClick={onFetchDiligence} disabled={diligenceBusy}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-violet-700 border border-[#9883FC] rounded-lg hover:bg-violet-50 disabled:opacity-40"
        ...
        <button onClick={onResetReference}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-500 border border-[#8C95A7] rounded-lg hover:bg-gray-50"
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <select value={saveSlug} onChange={e => setSaveSlug(e.target.value)}
            className="px-2 py-1.5 text-xs border border-[#8C95A7] rounded-lg text-gray-600">
      ...
      {notes.length > 0 && (
        <div className="mt-2 space-y-1">
          {notes.map((n, i) => (
            <p key={i} className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded px-2 py-1">{n}</p>
          ))}
        </div>
      )}
    </section>
