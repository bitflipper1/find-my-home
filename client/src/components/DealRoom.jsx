...
// magic links (see Monetization panel). Default passphrase: craig71
const GATE_HASH = '88e29b2689e841bd25a6ed36aa96fb10eb92c590557485316a6451481653cedc';
...
// Benchmark: the Craig Ave deal, from the signed contract (primary source)
const BENCHMARK = {
  label: '3912 Craig Ave (your signed deal)',
  price: 378987, incentive: 3153, rent: 2600, leaseMonths: 12,
...
export default function DealRoom({ market, research }) {
  ...
  const [d, setD] = useState({ address: '3916 Craig Ave, Charlotte, NC 28211', price: 400000, incentive: 20000, rent: 2600, leaseMonths: 12, hoa: 250, taxAnnual: 3100, sqft: 1600, isModel: true, isFurnished: true, isLeaseback: true });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <section className={`rounded-xl border p-4 ${beats ? 'bg-green-50 border-green-200' : 'bg-amber-50 border-amber-200'}`}>
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-xs text-gray-500"><th className="text-left font-medium py-1">Metric</th><th className="text-right font-medium">This deal</th><th className="text-right font-medium">Craig Ave benchmark</th></tr></thead>
          ...
          <p className={`flex items-start gap-1.5 text-xs font-medium rounded-lg p-2 mt-2 ${beats ? 'text-green-800 bg-green-100' : 'text-amber-800 bg-amber-100'}`}>
            ...
            {beats
              ...
              : `Below benchmark. To match Craig Ave $/sqft, effective price needs to be ~${fmt((bench.effective / BENCHMARK.sqft) * (d.sqft || 0))} — that's your counter.`}
          </p>
          ...
          <p className="text-xs text-red-700 mt-2">⚠ Leaseback = rental use: verify the builder's Investor Sales Addendum in the contract (your Craig Ave contract makes unauthorized rental use grounds for earnest forfeiture).</p>
        </section>
      </div>
